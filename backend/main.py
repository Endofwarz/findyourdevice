...
from backend.gsma_scraper import fetch_specs_live, fetch_price_live, fetch_gallery_urls
import traceback
...
# --- Constants ---
from backend.config import EUR_PER_USD
from backend.reddit_live import reddit_search_pros_cons  # add at top
from backend.dxomark_live import fetch_dxomark_camera_rank
...
from backend.llm import chat_complete
from backend.prompts import blurb_messages, pros_cons_messages
ALLOWED_ORIGINS = [
  "http://127.0.0.1:5173",
  "http://localhost:5173",
...
from backend.reddit_live import reddit_diag, reddit_search_pros_cons
...
# near other imports
from backend.dxomark_live import fetch_dxomark_camera_rank, diag_dxomark
...
def _seed_gsma_if_missing():
    try:
        if os.getenv("IMPORT_ON_BOOT", "1") == "1" and not os.path.exists(GSMA_OUT):
            from backend.gsma_scraper import bootstrap_import
...
# near other imports
from fastapi import HTTPException, Query
from backend.gsma_scraper import fetch_brand_since, ScrapeError
import pandas as pd, pathlib, csv, os
...
def import_gsma_brand(
    ...
    from backend.gsma_scraper import fetch_brand_since
...
def import_gsma_probe(brand: str):
    try:
        from backend.gsma_scraper import find_brand_url
...
from backend.gsma_scraper import fetch_specs_live  # <-- add at top of file
...
try:
    from backend.reddit_live import reddit_search_pros_cons as _reddit_search_pros_cons
except Exception:
...
def config_status():
    from backend.config import USE_LLM, PHONES_CSV
    return {
